...
import { tactileSpring, defaultShadow, dragSourceOpacity, overlaySpring, dragShadow } from '@/lib/animations';
...
const statusIconMap = {
  not_started: <Square className="h-4 w-4 text-muted-foreground" />,
...
export function SortableSection({ section }: SortableSectionProps) {
  ...
  const handleItemDragStart = (event: DragStartEvent) => {
    const item = section.items.find((i) => i.id === event.active.id);
    if (item) {
      setActiveItem(item);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 p-4 border-b border-border">
        ...
        <Button variant="ghost" size="sm" onClick={toggleCollapse} className="gap-2 shrink-0">
          {isCollapsed ? (
            <>
      ...
      {!isCollapsed && section.items.length > 0 && (
        <div className="p-4">
          ...
          >
            ...
            {/* Item DragOverlay — neon treatment */}
            <DragOverlay dropAnimation={null}>
              {activeItem ? (
                ...
                >
                  <div className="text-primary cursor-grabbing">
                  ...
                  <div
                    className={cn(
                      ...
                      activeItem.status === 'completed' && 'bg-success/10 border-success text-success',
                      activeItem.status === 'in_progress' && 'bg-primary/10 border-primary text-primary',
                      activeItem.status === 'not_started' && 'border-muted text-muted-foreground'
                    )}
                  >
                    {statusIconMap[activeItem.status]}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={cn(
                      ...
                      activeItem.status === 'completed' && 'line-through text-muted-foreground'
                    )}>
                      {activeItem.title}
                    </p>
                  </div>
